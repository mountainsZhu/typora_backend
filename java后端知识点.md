...
@Test
	...
	public void testCreateIndex() throws IOException {
		//创建索引请求
		CreateIndexRequest request = new CreateIndexRequest("zss_index");
		//客户端执行索引请求，并返回响应
...
@Test
	public void testGetIndex() throws IOException {
		//创建获取请求
		GetIndexRequest request = new GetIndexRequest("zss_index");
		//发送请求验证，返回bool值
...
@Test
	public void testDeleteIndex() throws IOException {
		DeleteIndexRequest request = new DeleteIndexRequest("zss_index");
		//客户端执行删除请求，返回删除状态信息
...
<img src="https://raw.githubusercontent.com/mountainsZhu/TyporaImage/main/img/20210101171915.PNG" style="zoom: 67%;" />
...
@Test                                                                                                    
	...
	public void testAddDocument() throws IOException {
		//创建对象
		User user = new User("zss",22);
		//创建请求
		IndexRequest request = new IndexRequest("zss_index");
		//插入数据 /index/_doc/id
		request.id("1");
		request.timeout(TimeValue.timeValueSeconds(2));
		//将数据放入请求json
		JSONObject jsonUser = JSONUtil.parseObj(user,false);
...
@Test
	...
	public void testIsExist() throws IOException {
		GetRequest request = new GetRequest("zss_index","1");
...
@Test
	...
	public void testUpdateDocument() throws IOException {
		UpdateRequest request = new UpdateRequest("zss_index","1");
		request.timeout(TimeValue.timeValueSeconds(2));
		User user = new User("zssnb",23);
		JSONObject userJson = JSONUtil.parseObj(user,false);
...
```java
	...
	public void testDeleteDocument() throws IOException {
		DeleteRequest request = new DeleteRequest("zss_index", "2");
		request.timeout(TimeValue.timeValueSeconds(2));
...
@Test
	...
	public void testBulkAddDocument() throws IOException {
		...
		list.add(new User("zss1",18));
		list.add(new User("zss2",19));
		list.add(new User("zss3",20));
		list.add(new User("zss4",15));
		list.add(new User("zss5",14));
		list.add(new User("zss6",13));
		list.add(new User("zss7",20));
		//批处理请求
		for (int i = 0; i < list.size(); i++) {
			bulkRequest.add(
					new IndexRequest("zss_index")
					.id(""+(i+1)) //如果不加id自动加上随机id
					.source(JSONUtil.parseObj(list.get(i),false),XContentType.JSON));
		}
...
@Test
	...
	public void testSearch() throws IOException {
		SearchRequest searchRequest = new SearchRequest("zss_index");
		SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
		//构建查询条件
		//精确匹配
		//TermQueryBuilder termQueryBuilder = QueryBuilders.termQuery("name", "zss1");
		//查全部
		MatchAllQueryBuilder matchAllQueryBuilder = QueryBuilders.matchAllQuery();
		//模糊匹配
		//MatchQueryBuilder matchQueryBuilder = QueryBuilders.matchQuery("name", "zss");
